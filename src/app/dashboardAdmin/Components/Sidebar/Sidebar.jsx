'use client'
import React, { useState } from 'react';
import Link from 'next/link';

export default function Sidebar() {
  const [sessionStyle, setSessionStyle] = React.useState({ background: '#121212' });

  const [profilStyle, setProfilStyle] = React.useState({ background: '#121212' });
  const [inscriptionStyle, setInscriptionStyle] = React.useState({ background: '#121212' });

  const handleSessionClick = () => {
    setSessionStyle({ background: 'linear-gradient(#121212, #F5F2FF)' });
    setProfilStyle({ background: '#121212' });
    setInscriptionStyle({ background: '#121212' });
  };

  

  const handleProfilClick = () => {
    setSessionStyle({ background: '#121212' });
    setProfilStyle({ background: 'linear-gradient(#121212, #F5F2FF)' });
    setInscriptionStyle({ background: '#121212' });
  };

  const handleInscriptionClick = () => {
    setSessionStyle({ background: '#121212' });
    setProfilStyle({ background: '#121212' });
    setInscriptionStyle({ background: 'linear-gradient(#121212, #F5F2FF)' });
  };

  return (
    <div className=" h-full flex flex-col w-full bg-[#121212] text-white border-r shadow-lg" style={{ fontFamily: 'NATS' }}>
      {/* Sidebar content */}
      {/* Logo */}
      <div className="flex items-center justify-center pt-5">
        <div className="w-20 h-16  text-lg flex items-center justify-center mb-4">
          LOGO
        </div>
      </div>

      {/* Dashboard */}
      <div className="w-full  h-14 text-md flex items-center justify-center mb-4" style={{ color: '#6610F2' }}>
        <div className="cursor-pointer rounded-2xl bg-[#EBEEF2] w-[90%] h-10 text-md flex items-center">
          <div className='mr-12 ml-8'>
          <img src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAACXBIWXMAAAsTAAALEwEAmpwYAAAAcElEQVR4nO2WMQrAMAwD/YPok/FLSuh3lamZ0qF0b4rBhcqg8cBgW5bZF6qCu4PD0eczcXhhi+JtDb5UwSOKt1X4VhRvasA1AmgJu85wJhsRk624sL14JlsUrzLlAZcRId2IuhqYGoFrCaE8wH/mgRNeCVM6SnSVDQAAAABJRU5ErkJggg==" style={{ width: 20, height: 20, marginLeft: 10 }} ></img>
           </div>
          Dashboard
        </div>
      </div>

      {/* Items */}
      <div className="flex-grow mt-3 text-md" style={{ color: 'white' }}>
        {/*Demande de création de session*/}
        <Link href="/dashboardAdmin/DemandeDeSession">
          <div
            className="border-b-2 border-[#E4D9FF] transition duration-300 ease-in-out cursor-pointer"
            style={sessionStyle}
            onClick={handleSessionClick}
          >
            <div className="mt-[5%] ml-[15%] mb-[5%] flex items-center">
              <div className="mr-[20%]">
                <img src="data:image/png;base64," style={{ width: 20, height: 20, marginLeft: 0 }} />
              </div>
              <span>Demande de session</span>
            </div>
          </div>
        </Link>
 
       
        {/*profil*/}
        <div
          className=" border-[#E4D9FF] border-b-2 transition duration-300 ease-in-out cursor-pointer"
          style={profilStyle}
          onClick={handleProfilClick}
        >
          <div className="mt-[5%] ml-[15%] mb-[5%] flex items-center">
            <div className="mr-[20%]">
              <img src="data:image/png;base64," style={{ width: 20, height: 20, marginLeft: 0 }} />
            </div>
            <span >Mon profil</span>
          </div>
        </div>

       {/* Demande d'inscription */}
        <Link href="/dashboardAdmin/DemandeDinscription">
        <div
          className="border-b-2 border-[#E4D9FF] transition duration-300 ease-in-out cursor-pointer"
          style={inscriptionStyle}
          onClick={handleInscriptionClick}
        >
          <div className=" mt-[5%] ml-[15%] mb-[5%]   flex items-center">
            <div className='mr-[20%]'>
                         <img src="data:image/png;base64,"style={{width: 20, height: 20,marginLeft: 0 }}></img> 
                </div>
            <span className=" boder-b-2 ">Demande d'inscription</span>
          </div>
        </div>
        </Link>
      </div>
      

      {/*button decconecter*/}
      <div className='flex justify-center hover:bg-[#E4D9FF] items-center border-2 bg-[#eee9f7] border-[#E4D9FF]   ml-[25%] w-32 h-10 rounded-xl mt-auto mb-4'>
        <button className="flex items-center text-black">
          Deconnecter
          <img src="data:image/png;base64," style={{ width: 20, height: 20, marginLeft: 10 }} />
        </button>
      </div>
</div>

   
  );
}
